...
nginx_log_path = ConfigManager().get_server_conf_by_key(product_name='sp', module_name='nginx', key='log_dir')

LOG_FILE = nginx_log_path + '/web.access.log.{data_time}'
...
def get_position(current_date, logger):
    log_file = LOG_FILE.format(data_time=current_date)
...
def handle_log(start_position, end_position, current_date, instance, logger):
    ...
    data = {}
    log = open(LOG_FILE.format(data_time=current_date))
    ...
    while True:
        ...
        try:
            # 这里过调掉无效请求
            if not any(url_case in line for url_case in valid_url):
                continue
            line = line.split(' ')
            http_code = line[10]
        except Exception:
    ...
    prometheus_helper.do_monitor_nginx(instance, 'sr_saas_project_all', data)
    print(data)
...
if __name__ == '__main__':
    ...
    logger.info('start gather saas nginx metric')
    start_position, end_position = get_position(current_date, logger)
    print([start_position, end_position])
    handle_log(start_position, end_position, current_date, instance, logger)
